...
$name = $age = $address = $email = $password = $confirm_password = "";
$name_err = $age_err = $address_err = $email_err = $password_err = $confirm_password_err = "";
$success_message = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["email"]))){
        ...
        if(!validate_email($email)) {
            ...
            if(check_email_exists($email, "patients")) {
    ...
    if(empty($name_err) && empty($age_err) && empty($address_err) && empty($email_err) && empty($password_err) && empty($confirm_password_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO patients (name, age, address, email, password) VALUES (?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sisss", $param_name, $param_age, $param_address, $param_email, $param_password);
